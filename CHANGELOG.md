...
### Added
- Build preprocessing now asks which panel to use when starting a build from the Unity editor.
- The selected build panel is saved before validation, then the normal checklist review, fix, skip, and build-blocking flow continues against that panel.

### Notes
- Batch mode builds keep using the currently active panel without showing a popup.
